  ...
  return (
    <div style={{ width: '100%', padding: '0 0 80px', color: TEXT, backgroundColor: 'var(--c-bg)' }}>
      ...
      <div style={{ padding: '24px 0 16px' }}>
        <div style={{ fontSize: 22, fontWeight: 900, color: TEXT }}>Mes cours</div>
        <div style={{ fontSize: 13, color: SUB, marginTop: 4 }}>{modules.length} chapitres</div>
      </div>
        ...
        {modules.map((mod, idx) => {
          ...
          return (
            <ChapterCard
              ...
              mascotId={mascotId}
              onContinue={() => router.push(`/progress?idx=${idx}`)}
            />
          );
        })}
      </div>
